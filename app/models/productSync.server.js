...
import { fetchZohoItemBySku, createZohoItem, updateZohoItem, setZohoItemActiveStatus, deleteZohoItem } from "../zoho.server";
...
export async function getSyncedProductCount(shopId) {
  const [rows] = await db.execute(`SELECT COUNT(*) AS count FROM sync_mappings WHERE shop_id = ? AND entity_type = ? AND status = 'synced'`, [shopId, ENTITY_TYPE]);
  return rows[0]?.count || 0;
}

export async function getProductMappings(shopId) {
  const [rows] = await db.execute(`SELECT shopify_id, zoho_id, status, last_synced_at, last_error FROM sync_mappings WHERE shop_id = ? AND entity_type = ?`, [shopId, ENTITY_TYPE]);
  return Object.fromEntries(rows.map((row) => [row.shopify_id, { zohoId: row.zoho_id, status: row.status, lastSyncedAt: row.last_synced_at, lastError: row.last_error }]));
}

export async function saveProductMapping(shopId, shopifyVariantId, zohoItemId, shopifyParentId) {
  await db.execute(`INSERT INTO sync_mappings (shop_id, entity_type, shopify_id, shopify_parent_id, zoho_id, status, last_synced_at, last_error)
     VALUES (?, ?, ?, ?, ?, 'synced', NOW(), NULL)
     ON DUPLICATE KEY UPDATE zoho_id = VALUES(zoho_id), shopify_parent_id = VALUES(shopify_parent_id), status = 'synced', last_synced_at = NOW(), last_error = NULL`, [shopId, ENTITY_TYPE, shopifyVariantId, shopifyParentId || null, zohoItemId]);
}

export async function markProductMappingError(shopId, shopifyVariantId, errorMessage) {
  await db.execute(`UPDATE sync_mappings SET status = 'error', last_error = ? WHERE shop_id = ? AND entity_type = ? AND shopify_id = ?`, [errorMessage, shopId, ENTITY_TYPE, shopifyVariantId]);
}

export async function getProductMappingsByParentId(shopId, shopifyParentId) {
  const [rows] = await db.execute(`SELECT shopify_id, zoho_id FROM sync_mappings WHERE shop_id = ? AND entity_type = ? AND shopify_parent_id = ?`, [shopId, ENTITY_TYPE, shopifyParentId]);
  return rows;
}

export async function deleteProductMapping(shopId, shopifyVariantId) {
  await db.execute(`DELETE FROM sync_mappings WHERE shop_id = ? AND entity_type = ? AND shopify_id = ?`, [shopId, ENTITY_TYPE, shopifyVariantId]);
}
...
export function buildZohoItemPayload(product, variant, { inventoryAccountId } = {}) {
  ...
  return { name, sku: variant.sku, rate: Number(variant.price) || 0, description: product.description || "", item_type: "inventory", product_type: "goods", ...(inventoryAccountId ? { track_inventory: true, inventory_account_id: inventoryAccountId } : {}) };
}
...
async function denyOversellForVariant(admin, product, variant) {
  ...
  const response = await admin.graphql(SET_VARIANT_INVENTORY_POLICY_MUTATION, { variables: { productId: product.id, variants: [{ id: variant.id, inventoryPolicy: "DENY" }] } });
...
export async function syncVariantToZoho({ shopId, admin, zohoAuth, product, variant, mappings, inventoryAccountId }) {
  ...
  try {
    ...
    if (zohoItemId) await updateZohoItem(zohoAuth, zohoItemId, payload);
    else {
      const existingItem = await fetchZohoItemBySku({ ...zohoAuth, sku: variant.sku });
      if (existingItem) { zohoItemId = existingItem.item_id; await updateZohoItem(zohoAuth, zohoItemId, payload); }
      else { const created = await createZohoItem(zohoAuth, payload); zohoItemId = created.item_id; }
    }
...
export async function syncProductToZoho({ shopId, admin, zohoAuth, product, mappings, inventoryAccountId }) {
  const results = [];
  for (const variant of product.variants) results.push(await syncVariantToZoho({ shopId, admin, zohoAuth, product, variant, mappings, inventoryAccountId }));
  return results;
}
...
export function normalizeProductNode(node) {
  return { ...node, imageUrl: node.featuredMedia?.preview?.image?.url || null, variants: (node.variants?.edges || []).map(({ node: variant }) => variant) };
}
...
async function fetchAllProductsForSync(admin) {
  ...
  for (;;) {
    ...
    const json = await response.json();
    allProducts.push(...(json.data?.products?.edges || []).map(({ node }) => normalizeProductNode(node)));
...
export async function runProductSync({ admin, shop, zohoAuth }) {
  ...
  const summary = { processed: attempted.length, success: attempted.filter((result) => result.status === "success").length, failed: attempted.filter((result) => result.status === "error").length };
  await finishSyncLog(logId, { recordsProcessed: summary.processed, recordsSuccess: summary.success, recordsFailed: summary.failed, metadata: attempted });
  return summary;
}
...
export function normalizeRestProduct(payload) {
  return {
    ...
    variants: (payload.variants || []).map((variant) => ({ id: variant.admin_graphql_api_id, title: variant.title, sku: variant.sku, price: variant.price })),
  };
}

export async function processProductUpsertWebhook({ shop: shopDomain, topic, webhookId, payload, admin }) {
  const { shop, connection } = await getConnectionForShopDomain(shopDomain);
  const logId = await recordWebhookReceived(shop.id, { webhookId, topic, shopDomain, resourceId: payload.admin_graphql_api_id, payload });
  if (!logId) return;
  if (!connection) {
    await finishWebhookLog(logId, { status: "skipped", errorMessage: "Zoho Books is not connected for this shop" });
    return;
  }
  try {
    ...
    const mappings = await getProductMappings(shop.id);
    const appSettings = await getAppSettings(shop.id);
    const results = await syncProductToZoho({ shopId: shop.id, admin, zohoAuth, product, mappings, inventoryAccountId: appSettings.accountSettings?.inventoryAccountId });
    const failed = results.filter((result) => result.status === "error");
    await finishWebhookLog(logId, { status: failed.length > 0 ? "failed" : "processed", errorMessage: failed.length > 0 ? failed.map((result) => `${result.sku}: ${result.error}`).join("; ") : null });
  } catch (error) {
    console.error("Failed to process product webhook", topic, error);
    await finishWebhookLog(logId, { status: "failed", errorMessage: error.message });
  }
}

export async function syncProductDeletionToZoho({ shopId, zohoAuth, shopifyProductId }) {
  ...
  for (const mapping of mappings) {
    try {
      ...
      try {
        ...
        console.error("Failed to delete or deactivate Zoho item for deleted product", mapping.zoho_id, deactivateError);
        await markProductMappingError(shopId, mapping.shopify_id, deactivateError.message);
        results.push({ zohoItemId: mapping.zoho_id, status: "error", error: deactivateError.message });
      }
    }
  }
  return results;
