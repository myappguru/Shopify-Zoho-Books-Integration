...
export default function Index() {
  ...
  const logs = syncStats.map((stat) => ({
    ...stat,
    log: recentLogs[stat.key],
    count: syncCounts[stat.key] || 0,
  }));
  ...
  const lastSync = logs
    ...
    .sort(
      (a, b) =>
        new Date(b.log.completed_at || b.log.started_at) -
        new Date(a.log.completed_at || a.log.started_at),
    )[0]?.log;
  ...
  return (
    <s-page heading="Dashboard" inlineSize="large">
      <style>{`
        ...
        .sync-row {
          display: grid;
          grid-template-columns: 105px minmax(80px, 1fr) 42px 72px;
          align-items: center;
          gap: 10px;
          padding: 9px 16px;
        }
      ...
      <div className="dashboard">
        <div className="dashboard-header">
          <div>
            ...
            <p className="dashboard-subtitle">
              Overview of your Shopify &amp; Zoho Books synchronization
            </p>
          </div>
            ...
            <div className="kpi-card" key={item.key}>
              <div className="kpi-top">
                <span className="kpi-label">
                  {labels[item.key]}
                  {item.key === "inventory" ? " Items" : ""}
                </span>
                <span className="kpi-icon">
                  <s-icon type={item.iconType} tone={item.iconTone}></s-icon>
                </span>
              </div>
              ...
              <div className="kpi-meta">
                ...
                <span>•</span>
                <span>
                  {getRelativeTime(item.log?.completed_at || item.log?.started_at)}
                </span>
              </div>
            </div>
          ))}
        ...
        <div className="connection-card">
          ...
          <div className="connection-center">
            <span className="connected-pill">
              ...
              {zohoConnected ? "Connected" : "Zoho not connected"}
            </span>
            <div className="connection-line"></div>
          </div>

          <div
            className="connection-side"
            style={{ justifyContent: "flex-end" }}
          >
            <div className="connection-right">
              <strong>
                {zohoConnected ? "Zoho Books" : "Connect Zoho Books"}
              </strong>
              <span>
                {zohoConnected
                  ? zohoOrganizationName || "Organization connected"
                  : "Authorize your Zoho organization to enable sync"}
              </span>
            ...
            <div className="connection-logo">
              <s-icon
                type="link"
                tone={zohoConnected ? "success" : "caution"}
              ></s-icon>
            </div>
          </div>
        </div>
        ...
        <div className="two-column">
          <div className="panel">
            <div className="panel-header">
              ...
              <p className="panel-subtitle">
                Latest synchronization success by data type
              </p>
            </div>

            {logs.map((item) => {
              const rate =
                getSuccessRate(item.log) ?? (item.count > 0 ? 100 : 0);
          ...
          <div className="panel">
            <div className="panel-header">
              ...
              <p className="panel-subtitle">
                Latest synchronization activities
              </p>
            </div>
                      ...
                      <div className="activity-row" key={item.key}>
                        ...
                        <div>
                          <div className="activity-title">
                            {labels[item.key]} sync {failed ? "partially completed" : "completed"}
                          </div>
                          <div className="activity-meta">
                            {formatCount(item.log.records_processed)} records processed · {formatCount(item.log.records_success)} succeeded
                          </div>
                        </div>
                        <span
                          className={`status-pill ${failed ? "status-partial" : ""}`}
                        >
                          {failed ? "Partial" : "Success"}
                        </span>
                        <span className="activity-time">
                          {formatDate(item.log.completed_at || item.log.started_at)}
                        </span>
                      </div>
                    );
                  })
            ...
            <div className="history-footer">
              <a href="/app/sync-history">View all activities →</a>
            </div>
          </div>
        </div>
        ...
        <div className="panel">
          <div className="panel-header">
            ...
            <p className="panel-subtitle">
              Summary of recent synchronization operations
            </p>
          </div>
          ...
          {hasActivity ? (
            <table className="history-table">
              ...
              <tbody>
                {logs
                  ...
                  .map((item) => {
                    const failed = Number(item.log.records_failed || 0) > 0;
                    const started = item.log.started_at
                      ? new Date(item.log.started_at)
                      : null;
                    const completed = item.log.completed_at
                      ? new Date(item.log.completed_at)
                      : null;
                    const duration =
                      started && completed
                    ...
                    return (
                      <tr key={item.key}>
                        ...
                        <td>{formatCount(item.log.records_processed)}</td>
                        <td>
                          <span
                            className={`status-pill ${failed ? "status-partial" : ""}`}
                          >
            ...
            <div className="empty-state">
              Your recent sync operations will appear here.
            </div>
          )}

          <div className="history-footer">
            <a href="/app/sync-history">View full sync history →</a>
          </div>
        </div>
        ...
        {lastSync && (
          <div className="last-sync">
            Last synchronization: {formatDate(lastSync.completed_at || lastSync.started_at)}
          </div>
        )}
      </div>
    </s-page>
