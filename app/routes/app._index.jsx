...
export default function Index() {
  ...
  return (
    <s-page heading="Dashboard">
      <style>{`
        .dashboard { display:flex; flex-direction:column; gap:18px; padding-bottom:24px; }
        ...
        .kpi-card,.panel,.connection-card { background:#fff; border:1px solid #e5e7eb; border-radius:14px; box-shadow:0 1px 2px rgba(0,0,0,.03); }
        .kpi-card { padding:18px; min-height:145px; display:flex; flex-direction:column; justify-content:space-between; }
        .kpi-top { display:flex; align-items:center; justify-content:space-between; gap:10px; }
        .kpi-label { font-size:14px; font-weight:600; color:#4b5563; }
        .kpi-icon { width:42px; height:42px; border-radius:12px; display:grid; place-items:center; background:#f1f5f9; }
        .kpi-number { font-size:30px; line-height:34px; font-weight:700; margin-top:8px; color:#111827; }
        .kpi-meta { display:flex; align-items:center; gap:7px; color:#6b7280; font-size:12px; }
        .success-text { color:#16a34a; font-weight:600; }
        .connection-card { padding:18px 20px; display:grid; grid-template-columns:1fr auto 1fr; align-items:center; gap:20px; }
        .connection-side { display:flex; align-items:center; gap:12px; min-width:0; }
        .connection-logo { width:46px; height:46px; border-radius:12px; display:grid; place-items:center; background:#f8fafc; border:1px solid #e5e7eb; }
        .connection-name { font-weight:700; font-size:16px; }
        .connection-caption { color:#6b7280; font-size:12px; margin-top:3px; }
        .connection-center { text-align:center; min-width:180px; }
        .connected-pill { display:inline-flex; align-items:center; gap:6px; background:#ecfdf3; color:#15803d; border-radius:999px; padding:6px 11px; font-size:12px; font-weight:700; }
        .connected-dot { width:7px; height:7px; border-radius:50%; background:#22c55e; }
        .connection-line { border-top:2px dashed #d1d5db; margin:10px 0 0; }
        .connection-right { text-align:right; }
        .connection-right strong { display:block; font-size:16px; }
        .connection-right span { color:#6b7280; font-size:12px; }
        .two-column { display:grid; grid-template-columns:minmax(0,1fr) minmax(0,1fr); gap:14px; }
        .panel { overflow:hidden; }
        .panel-header { padding:18px 18px 10px; }
        .panel-title { margin:0; font-size:16px; font-weight:700; color:#111827; }
        .panel-subtitle { margin:4px 0 0; color:#6b7280; font-size:12px; }
        .sync-row { display:grid; grid-template-columns:110px minmax(80px,1fr) 48px 80px; align-items:center; gap:12px; padding:11px 18px; }
        .sync-label { display:flex; align-items:center; gap:8px; font-size:13px; font-weight:600; }
        .sync-dot { width:8px; height:8px; border-radius:50%; background:#22c55e; }
        .progress-track { height:7px; background:#eef2f7; border-radius:999px; overflow:hidden; }
        .progress-fill { height:100%; background:#4f46e5; border-radius:999px; }
        .progress-value { font-size:12px; font-weight:700; text-align:right; }
        .progress-count { font-size:11px; color:#6b7280; text-align:right; }
        .activity-list { display:flex; flex-direction:column; }
        .activity-row { display:grid; grid-template-columns:24px minmax(0,1fr) auto auto; gap:10px; align-items:center; padding:11px 18px; border-top:1px solid #f0f1f3; }
        .activity-icon { width:24px; height:24px; border-radius:50%; display:grid; place-items:center; background:#ecfdf3; color:#16a34a; font-size:12px; font-weight:700; }
        .activity-title { font-size:13px; font-weight:600; color:#111827; }
        .activity-meta { font-size:11px; color:#6b7280; margin-top:2px; }
        .status-pill { padding:4px 8px; border-radius:999px; background:#ecfdf3; color:#15803d; font-size:11px; font-weight:700; }
        .status-partial { background:#fff7ed; color:#c2410c; }
        .activity-time { font-size:11px; color:#6b7280; white-space:nowrap; }
        .history-table { width:100%; border-collapse:collapse; }
        .history-table th { text-align:left; padding:10px 18px; background:#f8fafc; color:#6b7280; font-size:11px; font-weight:700; text-transform:uppercase; letter-spacing:.04em; }
        .history-table td { padding:12px 18px; border-top:1px solid #f0f1f3; font-size:12px; color:#374151; }
        .history-type { display:flex; align-items:center; gap:8px; font-weight:600; color:#111827; }
        .history-footer { padding:13px 18px; border-top:1px solid #f0f1f3; text-align:center; }
        .history-footer a { color:#4338ca; font-size:12px; font-weight:600; text-decoration:none; }
        .empty-state { padding:28px 18px; color:#6b7280; font-size:13px; text-align:center; }
        ...
        @media (max-width: 620px) { .kpi-grid,.two-column { grid-template-columns:1fr; } .dashboard-header { flex-direction:column; } .sync-row { grid-template-columns:100px 1fr 45px; } .progress-count { display:none; } .activity-row { grid-template-columns:24px 1fr auto; } .activity-time { display:none; } }
      `}</style>
      ...
      <div className="dashboard">
        ...
        {lastSync && <div style={{ color: "#6b7280", fontSize: 12 }}>Last synchronization: {formatDate(lastSync.completed_at || lastSync.started_at)}</div>}
      </div>
    </s-page>
  );
}
