...
export default function ProductsPage() {
  ...
  const syncingProductId =
    navigation.state === "submitting" &&
    navigation.formData?.get("intent") === "sync-product"
      ? navigation.formData?.get("productId")
      : null;
  ...
  const visibleProducts = useMemo(() => {
    ...
    return products.filter((product) => {
      ...
      const matchesSku =
        skuFilter === "all" ||
        (skuFilter === "with-sku" && hasSku) ||
        (skuFilter === "without-sku" && !hasSku);

      const matchesVariants =
        variantFilter === "all" ||
        (variantFilter === "single" && variants.length === 1) ||
        (variantFilter === "multiple" && variants.length > 1);
    ...
    <s-page heading="Products" inlineSize="large">
      <style>{`
        ...
        .toolbar {
          padding: 12px 14px;
          display: grid;
          grid-template-columns: minmax(0, 2.1fr) minmax(170px, 1fr) minmax(170px, .98fr) auto;
          gap: 10px;
          align-items: center;
        }
        .search-box {
        ...
        .product-table th:nth-child(5), .product-table td:nth-child(5) { width: 13%; }
        .product-table th:nth-child(6), .product-table td:nth-child(6) { width: 14%; }
        .product-table th:nth-child(7), .product-table td:nth-child(7) { width: 6%; }
        ...
        .action-wrap { position: relative; display: flex; justify-content: flex-start; }
        ...
        @media (max-width: 1100px) {
          .products-page { width: 90%; }
          .toolbar { grid-template-columns: minmax(220px, 1.5fr) minmax(150px, 1fr) minmax(150px, 1fr) auto; }
        }
        ...
        @media (max-width: 620px) {
          ...
          .more-filter-panel { left: 0; right: auto; width: min(300px, calc(100vw - 40px)); }
        }
      `}</style>
      ...
      <div className="products-page">
        <div className="products-header">
          <div>
            ...
            <p className="products-subtitle">
              Manage and sync your Shopify products with Zoho Books
            </p>
          </div>
          <div className="products-actions">
            <Form method="get" onSubmit={() => setIsRefreshing(true)}>
              <s-button type="submit" icon="refresh" loading={isRefreshing}>
                Refresh
              </s-button>
            </Form>
            <Form method="post">
        ...
        <div className="stats-grid">
          <div className="products-card stat-card">
            <div className="stat-top"><span className="stat-label">Products on page</span><span className="stat-icon"><s-icon type="product" tone="info"></s-icon></span></div>
            <div className="stat-number">{formatCount(stats.products)}</div>
            <div className="stat-caption">Currently loaded from Shopify</div>
          </div>
          <div className="products-card stat-card">
            <div className="stat-top"><span className="stat-label">Synced to Zoho</span><span className="stat-icon"><s-icon type="check-circle" tone="success"></s-icon></span></div>
            <div className="stat-number">{formatCount(stats.synced)}</div>
            <div className="stat-caption">Variants successfully synced</div>
          </div>
          <div className="products-card stat-card">
            <div className="stat-top"><span className="stat-label">Not Synced</span><span className="stat-icon"><s-icon type="clock" tone="caution"></s-icon></span></div>
            <div className="stat-number">{formatCount(stats.unsynced)}</div>
            <div className="stat-caption">Pending synchronization</div>
          </div>
          <div className="products-card stat-card">
            <div className="stat-top"><span className="stat-label">Sync Errors</span><span className="stat-icon"><s-icon type="alert-circle" tone="critical"></s-icon></span></div>
            <div className="stat-number">{formatCount(stats.errors)}</div>
            <div className="stat-caption">Failed product variants</div>
          </div>
        </div>

        <div className="products-card toolbar-card">
          <div className="toolbar">
            <label className="search-box">
              ...
              <input
                ...
                placeholder="Search products by title, SKU, or handle"
            ...
            <select className="filter-select" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} aria-label="Filter by sync status">
              <option value="all">Sync Status · All</option>
              <option value="synced">Sync Status · Synced</option>
              <option value="unsynced">Sync Status · Not Synced</option>
              <option value="error">Sync Status · Errors</option>
            </select>
            ...
            <select className="filter-select" value={inventoryFilter} onChange={(event) => setInventoryFilter(event.target.value)} aria-label="Filter by inventory status">
              ...
              <option value="in-stock">Inventory · In stock</option>
              <option value="out-of-stock">Inventory · Out of stock</option>
            </select>

            <div className="more-filter-wrap">
              <s-button icon="filter" onClick={() => { setShowMoreFilters((value) => !value); setOpenActionId(null); }}>
                More filters{activeMoreFilterCount > 0 && <span className="filter-count">{activeMoreFilterCount}</span>}
              </s-button>
              {showMoreFilters && (
                <div className="more-filter-panel">
                  <div className="more-filter-title">
                    <span>More filters</span>
                    <s-button variant="tertiary" onClick={() => setShowMoreFilters(false)}>Close</s-button>
                  </div>
                  <div className="more-filter-group">
                    <label className="more-filter-label">Product status</label>
                    <select className="filter-select" value={productStatusFilter} onChange={(event) => setProductStatusFilter(event.target.value)}>
                  ...
                  <div className="more-filter-group">
                    <label className="more-filter-label">SKU</label>
                    <select className="filter-select" value={skuFilter} onChange={(event) => setSkuFilter(event.target.value)}>
                      <option value="all">All products</option>
                      <option value="with-sku">With SKU</option>
                      <option value="without-sku">Without SKU</option>
                    </select>
                  </div>
                  <div className="more-filter-group">
                    <label className="more-filter-label">Variants</label>
                    <select className="filter-select" value={variantFilter} onChange={(event) => setVariantFilter(event.target.value)}>
                      <option value="all">All products</option>
                      <option value="single">Single variant</option>
                      <option value="multiple">Multiple variants</option>
                    </select>
                  </div>
                  <div className="more-filter-footer">
                    <button type="button" className="clear-filter" onClick={clearMoreFilters}>Clear filters</button>
                    <s-button variant="primary" onClick={() => setShowMoreFilters(false)}>Done</s-button>
                  </div>
                </div>
              )}
            </div>
              ...
              <table className="product-table">
                ...
                <tbody>
                  {visibleProducts.map((product) => {
                    ...
                    const productMappings = variants
                      .map((variant) => mappings[variant.id])
                      .filter(Boolean);
                    ...
                    const isThisProductSyncing = syncingProductId === product.id;
                    ...
                    return (
                      <tr key={product.id}>
                        <td>
                          <div className="product-cell">
                            {product.imageUrl ? (
                              ...
                              <div className="product-image product-image-placeholder"><s-icon type="image"></s-icon></div>
                            )}
                            <div className="product-info">
                              <a className="product-name" href={`shopify://admin/products/${shopifyNumericId(product.id)}`} target="_top">{product.title}</a>
                              <div className="product-sub">{variants.length} {variants.length === 1 ? "variant" : "variants"} · {product.status}</div>
                            </div>
                          </div>
                        </td>
                        ...
                        <td>
                          <span className="inventory-number">{formatCount(quantity)}</span>
                          <span className={`inventory-label ${quantity > 0 ? "inventory-good" : "inventory-empty"}`}>{quantity > 0 ? "In stock" : "Out of stock"}</span>
                        </td>
                        <td className="status-cell">
                          <span className={`status-badge status-${status}`}>
                            <s-icon type={status === "synced" ? "check-circle" : status === "error" ? "alert-circle" : "clock"}></s-icon>
                            {statusLabel}
                          </span>
                        </td>
                        ...
                        <td>
                          <div className="action-wrap">
                            <button className="action-button" type="button" aria-label={`Actions for ${product.title}`} title="More actions" onClick={() => { setOpenActionId((value) => value === product.id ? null : product.id); setShowMoreFilters(false); }}>
                              •••
                            </button>
                            {openActionId === product.id && (
                              <div className="action-menu">
                                <a href={`shopify://admin/products/${shopifyNumericId(product.id)}`} target="_top">View in Shopify</a>
                                <Form method="post" onSubmit={() => setOpenActionId(null)}>
                                  <input type="hidden" name="intent" value="sync-product" />
                                  <input type="hidden" name="productId" value={product.id} />
                                  <button type="submit" disabled={Boolean(syncingProductId)}>
                                    {isThisProductSyncing ? "Syncing product…" : "Sync this product"}
                                  </button>
                                </Form>
                              </div>
                            )}
          ...
          <div className="table-footer">
            ...
            {(pageInfo.hasPreviousPage || pageInfo.hasNextPage) && (
              <div className="pagination">
                <s-button variant="secondary" disabled={!pageInfo.hasPreviousPage || isRefreshing} href={pageInfo.hasPreviousPage ? `?before=${encodeURIComponent(pageInfo.startCursor)}` : undefined}>‹</s-button>
                <span>Current</span>
                <s-button variant="secondary" disabled={!pageInfo.hasNextPage || isRefreshing} href={pageInfo.hasNextPage ? `?after=${encodeURIComponent(pageInfo.endCursor)}` : undefined}>›</s-button>
              </div>
            )}
          </div>
        </div>
