...
export const action = async ({ request }) => {
  ...
  const token = await getValidAccessToken(shop.id).catch((error) => {
    console.error("Failed to get a valid Zoho access token", error);
    return null;
  });
  ...
  if (intent === "sync-product") {
    ...
    await syncProductToZoho({
      ...
      admin,
      zohoAuth,
      product: normalizeProductNode(product),
      mappings,
...
function getVariantSummary(product) {
  ...
  return { variants, quantity: variants.reduce((total, variant) => total + Number(variant.inventoryQuantity || 0), 0) };
}
  ...
  return (
    <s-page heading="Products" inlineSize="large">
      <style>{`
        ...
        .products-header{display:flex;justify-content:space-between;align-items:flex-start;gap:16px;margin-bottom:16px}.products-title{margin:0;font-size:20px;line-height:26px;font-weight:650;letter-spacing:-.01em}.products-subtitle{margin:3px 0 0;color:#616161;font-size:15px;line-height:21px}.products-actions{display:flex;align-items:center;gap:8px;flex-shrink:0}.products-card{background:#fff;border:1px solid #dfe3e8;border-radius:10px;box-shadow:0 1px 1px rgba(0,0,0,.03);overflow:visible}.stats-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-bottom:12px}.stat-card{padding:14px 16px;min-height:108px;display:flex;flex-direction:column;justify-content:space-between;overflow:hidden}.stat-top{display:flex;justify-content:space-between;align-items:center;gap:12px}.stat-label{font-size:12px;line-height:16px;font-weight:600;color:#303030}.stat-icon{width:38px;height:38px;border-radius:9px;display:grid;place-items:center;background:#eef6ff}.stat-icon s-icon{width:20px;height:20px}.stat-card:nth-child(2) .stat-icon{background:#e8f7ed}.stat-card:nth-child(3) .stat-icon{background:#fff5e5}.stat-card:nth-child(4) .stat-icon{background:#f3ebff}.stat-number{font-size:26px;line-height:30px;font-weight:650;margin-top:4px;letter-spacing:-.02em}.stat-caption{color:#6d7175;font-size:13px;line-height:18px}.sync-banner{margin-bottom:12px}.toolbar{padding:12px 14px;display:grid;grid-template-columns:minmax(260px,1.6fr) repeat(3,minmax(130px,.65fr));gap:10px;align-items:center}.search-box{height:40px;border:1px solid #c9cdd1;border-radius:8px;display:flex;align-items:center;gap:8px;padding:0 12px;background:#fff;box-sizing:border-box}.search-box:focus-within{border-color:#005bd3;box-shadow:0 0 0 1px #005bd3}.search-box s-icon{width:17px;height:17px;flex:0 0 17px}.search-box input{border:0;outline:0;width:100%;font:inherit;color:#202223;font-size:13px;background:transparent}.filter-select{height:40px;border:1px solid #c9cdd1;border-radius:8px;padding:0 10px;background:#fff;color:#303030;font:inherit;font-size:13px;width:100%}
        .more-filter-wrap{position:relative}.more-filter-panel{position:absolute;z-index:100;top:48px;right:0;width:300px;padding:14px;background:#fff;border:1px solid #dfe3e8;border-radius:10px;box-shadow:0 8px 24px rgba(0,0,0,.12)}.more-filter-title{display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;font-size:13px;font-weight:650}.more-filter-group{margin-bottom:12px}.more-filter-label{display:block;margin-bottom:6px;color:#616161;font-size:11px;line-height:15px;font-weight:650}.more-filter-footer{display:flex;justify-content:space-between;align-items:center;padding-top:10px;border-top:1px solid #edf0f2}.clear-filter{border:0;background:none;color:#005bd3;cursor:pointer;font-size:12px}.filter-count{display:inline-flex;align-items:center;justify-content:center;min-width:18px;height:18px;padding:0 5px;border-radius:999px;background:#202223;color:#fff;font-size:10px;line-height:18px;margin-left:5px}
        .table-wrap{overflow-x:auto}.product-table{width:100%;min-width:850px;border-collapse:collapse}.product-table th{text-align:left;background:#f6f7f8;color:#6d7175;padding:10px 14px;font-size:10px;line-height:15px;text-transform:uppercase;letter-spacing:.035em;font-weight:650;border-bottom:1px solid #e5e7e9}.product-table td{padding:11px 14px;border-bottom:1px solid #edf0f2;font-size:13px;line-height:18px;color:#303030;vertical-align:middle}.product-table tr:last-child td{border-bottom:0}.product-cell{display:flex;align-items:center;gap:10px;min-width:0}.product-image{width:42px;height:42px;border-radius:7px;border:1px solid #e1e4e6;object-fit:cover;background:#f6f6f7;flex:0 0 42px}.product-image-placeholder{display:grid;place-items:center;color:#8c9196}.product-info{min-width:0}.product-name{display:block;color:#202223;font-weight:650;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-decoration:none}.product-name:hover{text-decoration:underline}.product-sub{color:#6d7175;font-size:11px;line-height:16px;margin-top:1px}.sku{font-size:12px;color:#303030}.muted{color:#6d7175}.inventory-number{display:block;font-weight:600}.inventory-label{display:block;font-size:11px;line-height:16px;margin-top:1px}.inventory-good{color:#008060}.inventory-empty{color:#d72c0d}.status-badge{display:inline-flex;align-items:center;gap:5px;padding:4px 8px;border-radius:6px;font-size:11px;line-height:15px;font-weight:650;white-space:nowrap}.status-synced{background:#e3f7ed;color:#008060}.status-unsynced{background:#fff5e5;color:#b98900}.status-error{background:#fdecea;color:#d72c0d}.status-badge s-icon{width:13px;height:13px}
        .action-wrap{position:relative;display:flex;justify-content:flex-end}.action-button{width:32px;height:32px;border:1px solid #dfe3e8;border-radius:7px;background:#fff;cursor:pointer;display:grid;place-items:center;color:#303030;font-weight:700;letter-spacing:1px}.action-button:hover{background:#f6f7f8}.action-menu{position:absolute;z-index:100;top:38px;right:0;width:190px;padding:6px;background:#fff;border:1px solid #dfe3e8;border-radius:9px;box-shadow:0 8px 24px rgba(0,0,0,.12)}.action-menu a,.action-menu button{width:100%;display:flex;align-items:center;gap:8px;padding:9px 10px;border:0;border-radius:6px;background:transparent;color:#202223;text-align:left;text-decoration:none;font:inherit;font-size:12px;cursor:pointer}.action-menu a:hover,.action-menu button:hover{background:#f6f7f8}.action-menu button:disabled{opacity:.5;cursor:not-allowed}.empty-products{padding:44px 20px;text-align:center;color:#6d7175;font-size:13px}.table-footer{display:flex;justify-content:space-between;align-items:center;gap:12px;padding:12px 14px;border-top:1px solid #e5e7e9;color:#6d7175;font-size:13px;line-height:18px}.pagination{display:flex;align-items:center;gap:6px}.sync-note{margin-top:12px;color:#6d7175;font-size:13px;line-height:18px}
        @media(max-width:900px){.products-page{width:100%}.stats-grid{grid-template-columns:1fr 1fr}.toolbar{grid-template-columns:1fr 1fr}.products-header{flex-direction:column}}@media(max-width:620px){.stats-grid{grid-template-columns:1fr}.toolbar{grid-template-columns:1fr}.table-footer{flex-direction:column;align-items:flex-start}.more-filter-panel{right:auto;left:0}}
      `}</style>
      ...
      <div className="products-page">
        ...
        <div className="products-card" style={{ marginBottom: "12px", overflow: "visible" }}><div className="toolbar"><label className="search-box"><s-icon type="search"></s-icon><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search products by title, SKU, or handle" aria-label="Search products" /></label><select className="filter-select" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="all">Sync Status · All</option><option value="synced">Sync Status · Synced</option><option value="unsynced">Sync Status · Not Synced</option><option value="error">Sync Status · Errors</option></select><select className="filter-select" value={inventoryFilter} onChange={(event) => setInventoryFilter(event.target.value)}><option value="all">Inventory · All</option><option value="in-stock">Inventory · In stock</option><option value="out-of-stock">Inventory · Out of stock</option></select><div className="more-filter-wrap"><s-button icon="filter" onClick={() => setShowMoreFilters((open) => !open)}>More filters{activeMoreFilterCount > 0 ? <span className="filter-count">{activeMoreFilterCount}</span> : null}</s-button>{showMoreFilters && <div className="more-filter-panel" role="dialog" aria-label="More product filters"><div className="more-filter-title"><span>More filters</span>{activeMoreFilterCount > 0 ? <button className="clear-filter" type="button" onClick={clearMoreFilters}>Clear</button> : null}</div><div className="more-filter-group"><label className="more-filter-label" htmlFor="product-status-filter">Product status</label><select id="product-status-filter" className="filter-select" value={productStatusFilter} onChange={(event) => setProductStatusFilter(event.target.value)}><option value="all">All statuses</option><option value="active">Active</option><option value="draft">Draft</option><option value="archived">Archived</option></select></div><div className="more-filter-group"><label className="more-filter-label" htmlFor="sku-filter">SKU</label><select id="sku-filter" className="filter-select" value={skuFilter} onChange={(event) => setSkuFilter(event.target.value)}><option value="all">All products</option><option value="with-sku">With SKU</option><option value="without-sku">Without SKU</option></select></div><div className="more-filter-group"><label className="more-filter-label" htmlFor="variant-filter">Variants</label><select id="variant-filter" className="filter-select" value={variantFilter} onChange={(event) => setVariantFilter(event.target.value)}><option value="all">All products</option><option value="single">Single variant</option><option value="multiple">Multiple variants</option></select></div><div className="more-filter-footer"><span className="muted">Filters apply instantly</span><s-button onClick={() => setShowMoreFilters(false)}>Done</s-button></div></div>}</div></div></div>

        <div className="products-card">{visibleProducts.length === 0 ? <div className="empty-products">{products.length === 0 ? "No products found in this store." : "No products match your current search or filters."}</div> : <div className="table-wrap"><table className="product-table"><thead><tr><th style={{width:"31%"}}>Product</th><th style={{width:"15%"}}>SKU</th><th style={{width:"11%"}}>Price</th><th style={{width:"13%"}}>Inventory</th><th style={{width:"13%"}}>Sync Status</th><th style={{width:"12%"}}>Last Synced</th><th style={{width:"5%"}}>Action</th></tr></thead><tbody>{visibleProducts.map((product) => { const { quantity, variants } = getVariantSummary(product); const productMappings = variants.map((variant) => mappings[variant.id]).filter(Boolean); const hasError = productMappings.some((mapping) => mapping.status === "error"); const allSynced = variants.length > 0 && variants.every((variant) => mappings[variant.id]?.status === "synced"); const status = hasError ? "error" : allSynced ? "synced" : "unsynced"; const lastSynced = productMappings.map((mapping) => mapping.lastSyncedAt).filter(Boolean).sort().pop(); const primaryVariant = variants[0]; const statusLabel = status === "error" ? "Sync error" : status === "synced" ? "Synced" : "Not synced"; const isThisProductSyncing = syncingProductId === product.id; return <tr key={product.id}><td><div className="product-cell">{product.imageUrl ? <img className="product-image" src={product.imageUrl} alt={product.title} /> : <div className="product-image product-image-placeholder"><s-icon type="image"></s-icon></div>}<div className="product-info"><a className="product-name" href={`shopify://admin/products/${shopifyNumericId(product.id)}`} target="_top">{product.title}</a><div className="product-sub">{variants.length} {variants.length === 1 ? "variant" : "variants"} · {product.status}</div></div></div></td><td><span className="sku">{primaryVariant?.sku || <span className="muted">No SKU</span>}</span></td><td>{primaryVariant ? formatPrice(primaryVariant.price) : "—"}</td><td><span className="inventory-number">{formatCount(quantity)}</span><span className={`inventory-label ${quantity > 0 ? "inventory-good" : "inventory-empty"}`}>{quantity > 0 ? "In stock" : "Out of stock"}</span></td><td><span className={`status-badge status-${status}`}><s-icon type={status === "synced" ? "check-circle" : status === "error" ? "alert-circle" : "clock"}></s-icon>{statusLabel}</span></td><td className="muted">{formatDate(lastSynced)}</td><td><div className="action-wrap"><button className="action-button" type="button" aria-label={`Actions for ${product.title}`} onClick={() => setOpenActionId((current) => current === product.id ? null : product.id)}>•••</button>{openActionId === product.id && <div className="action-menu" role="menu"><a href={`shopify://admin/products/${shopifyNumericId(product.id)}`} target="_top" role="menuitem">View in Shopify</a><Form method="post" onSubmit={() => setOpenActionId(null)}><input type="hidden" name="intent" value="sync-product" /><input type="hidden" name="productId" value={product.id} /><button type="submit" disabled={!connected || isThisProductSyncing} role="menuitem">{isThisProductSyncing ? "Syncing product…" : "Sync this product"}</button></Form></div>}</div></td></tr>; })}</tbody></table></div>}
          <div className="table-footer"><span>Showing {visibleProducts.length} of {products.length} products on this page</span>{(pageInfo.hasPreviousPage || pageInfo.hasNextPage) && <div className="pagination"><s-button variant="secondary" disabled={!pageInfo.hasPreviousPage || isRefreshing} href={pageInfo.hasPreviousPage ? `?before=${encodeURIComponent(pageInfo.startCursor)}` : undefined}>‹</s-button><span>Current page</span><s-button variant="secondary" disabled={!pageInfo.hasNextPage || isRefreshing} href={pageInfo.hasNextPage ? `?after=${encodeURIComponent(pageInfo.endCursor)}` : undefined}>›</s-button></div>}</div>
        </div>
