...
function OrganizationSection({ organization, refreshing }) {
  return (
    <>
      ...
      <div className="meta-row"><span>Last refreshed on {formatDate(organization.fetchedAt)}</span><span>Data is refreshed every 24 hours</span></div>
...
function ConnectionSection({ data, testing }) {
  ...
  const token = data.connection?.tokenMasked || "zoho••••••••••••••••••••••••••••••••";
  const organization = data.organization || {};
  const tokenValid = data.connection?.tokenExpiresAt ? new Date(data.connection.tokenExpiresAt).getTime() > Date.now() : connected;
  const zohoBooksUrl = data.connection?.dataCenter ? `https://books.zoho.${data.connection.dataCenter}` : "https://www.zoho.com/books/";
  const copyToken = async () => { try { await navigator.clipboard.writeText(token); setCopied(true); setTimeout(() => setCopied(false), 1600); } catch { } };
  return (
    <>
      <div className="content-head"><div><h2 className="content-title">Zoho Books Connection</h2><p className="content-sub">Connect your Zoho Books organization and manage the connection settings.</p></div><a className="outline-btn" href={data.zohoAuthUrl || "#"}><Icon type="refresh" />Reconnect</a></div>
      <div className="connection-page">
        {connected ? <div className="connection-status"><div className="connection-status-main"><span className="success-icon"><Icon type="check-circle" /></span><div><div className="status-title">Connected to Zoho Books</div><div className="status-copy">Your store is successfully connected to Zoho Books.</div></div></div><div className="status-meta"><span>Connected on</span><strong>{formatDate(data.connection.connectedAt)}</strong></div><div className="status-meta"><span>Connected by</span><strong>{display(data.connection.connectedBy)}</strong></div></div> : <div className="connection-status" style={{ borderColor: "#f2d79a", background: "#fffaf0" }}><div className="connection-status-main"><span className="success-icon" style={{ background: "#d88900" }}><Icon type="link" /></span><div><div className="status-title" style={{ color: "#8a5a00" }}>Not connected to Zoho Books</div><div className="status-copy">Connect your Zoho Books organization to enable synchronization.</div></div></div></div>}
        <div className="connection-box"><div className="box-head"><span className="box-title">Zoho Organization Details</span><a className="outline-btn" href={zohoBooksUrl} target="_blank" rel="noreferrer">View in Zoho Books <Icon type="external" /></a></div><div className="org-grid"><OrgCell icon="organization" label="Organization Name" value={display(organization.organizationName, data.connection?.organizationName)} /><OrgCell icon="hashtag" label="Zoho Books Organization ID" value={display(organization.organizationId, data.connection?.organizationId)} /><OrgCell icon="plan" label="Books Edition" value={display(organization.planName, "Professional")} /><OrgCell icon="cash-dollar" tone="orange" label="Base Currency" value={`${display(organization.currencyCode)}${organization.currencySymbol ? ` - ${organization.currencySymbol}` : ""}`} /><OrgCell icon="globe" tone="orange" label="Zoho Books Region" value={data.connection?.dataCenter ? `${String(data.connection.dataCenter).toUpperCase()} Data Center` : "—"} /><OrgCell icon="calendar" tone="purple" label="Financial Year" value={fiscalYear(organization.fiscalYearStartMonth)} /></div></div>
        <div className="connection-box"><div className="box-head"><div><div className="box-title">Access Token</div><div className="content-sub">Secure access token used to authenticate API requests.</div></div></div><div className="token-box"><div className="token-row"><div className="token-field"><span className="token-value">{showToken ? token : token.replace(/[^•]/g, "•")}</span><button className="token-eye" type="button" onClick={() => setShowToken((v) => !v)}><Icon type="view" /></button></div><button className="token-copy" type="button" onClick={copyToken}>{copied ? "Copied" : "Copy"}</button><span className="valid">{tokenValid ? "Valid" : "Expired"}</span></div><div style={{ marginTop: 10, fontSize: 10, color: "#596a87" }}>Token expiry: {formatDate(data.connection?.tokenExpiresAt)}</div></div></div>
        <div className="connection-box"><div className="box-head"><div><div className="box-title">Sync Preferences</div><div className="content-sub">Choose what data you want to sync with Zoho Books.</div></div></div><SyncPreferences data={data} /></div>
      </div>
      <div className="content-footer"><Form method="post"><input type="hidden" name="intent" value="test-connection" /><button className="secondary-btn" type="submit" disabled={testing}>{testing ? "Testing…" : "Test Connection"}</button></Form><Form method="post"><input type="hidden" name="intent" value="save-sync-preferences" /><input type="hidden" name="productsEnabled" value="true" /><button className="primary-btn" type="submit">Save Changes</button></Form></div>
    </>
  );
}
function OrgCell({ icon, tone = "blue", label, value }) { return <div className="org-cell"><div className={`mini-icon ${tone}`}><Icon type={icon} /></div><div className="mini-copy"><span>{label}</span><strong>{display(value)}</strong></div></div>; }
function SyncPreferences({ data }) { const prefs = data.syncPreferences || { products: true, orders: true, customers: true }; return <div className="sync-box"><SyncRow name="Products" description="Sync products and inventory to Zoho Books." icon="product" checked={prefs.products !== false} /><SyncRow name="Orders" description="Sync orders as invoices/sales orders." icon="order" tone="orange" checked={prefs.orders !== false} /><SyncRow name="Customers" description="Sync customers to Zoho Books." icon="customer" tone="purple" checked={prefs.customers !== false} /></div>; }
function SyncRow({ name, description, icon, tone = "blue", checked }) { return <div className="sync-row"><span className={`sync-icon ${tone}`}><Icon type={icon} /></span><div className="sync-copy"><strong>{name}</strong><span>{description}</span></div><label className="switch"><input type="checkbox" defaultChecked={checked} /><span className="slider" /></label></div>; }

function TaxSection({ data }) {
  const taxes = data.taxes || []; const rows = data.taxRateRows || []; const settings = data.taxSettings || {};
  const mappedCount = rows.filter((row) => Boolean(settings.rateMap?.[row.key])).length; const unmappedCount = Math.max(rows.length - mappedCount, 0);
  const defaultTax = taxes.find((tax) => String(tax.tax_id || tax.id) === String(settings.defaultTaxId || "")); const taxName = (tax) => tax?.tax_name || tax?.name || "—"; const taxId = (tax) => tax?.tax_id || tax?.id || "—"; const taxRate = (tax) => tax?.tax_percentage ?? tax?.rate ?? tax?.percentage; const taxType = (tax) => tax?.tax_type || tax?.tax_type_formatted || "Goods & Services Tax"; const codeFor = (row) => row.code || String(row.key || "").toUpperCase().replace(/[^A-Z0-9]+/g, ""); const selectedTax = (row) => taxes.find((tax) => String(taxId(tax)) === String(settings.rateMap?.[row.key] || ""));
  const [tab, setTab] = useState("mapping");
  return <div className="tax-section"><div className="tax-head"><div><h2 className="tax-title">Tax Settings</h2><p className="tax-sub">Configure how taxes are synced from Shopify to Zoho Books.</p></div><Form method="post"><input type="hidden" name="intent" value="refresh-zoho-data" /><button className="refresh-zoho tax-refresh" type="submit"><Icon type="refresh" />Refresh from Zoho</button></Form></div><Form method="post" className="tax-form"><input type="hidden" name="intent" value="save-tax-settings" /><div className="tax-info"><Icon type="info" />Map your Shopify tax settings to Zoho Books tax accounts for accurate tax calculation and reporting.</div><div className="tax-summary"><div className="tax-summary-card"><div className="tax-summary-icon"><Icon type="receipt" /></div><div className="tax-summary-copy"><span>Total Tax Rates</span><strong>{rows.length}</strong><small>Active tax rates</small></div></div><div className="tax-summary-card"><div className="tax-summary-icon green"><Icon type="check-circle" /></div><div className="tax-summary-copy"><span>Mapped Tax Rates</span><strong>{mappedCount}</strong><small className="green-text">{rows.length ? `${Math.round((mappedCount / rows.length) * 100)}% mapped` : "0% mapped"}</small></div></div><div className="tax-summary-card"><div className="tax-summary-icon orange"><Icon type="alert-circle" /></div><div className="tax-summary-copy"><span>Unmapped Tax Rates</span><strong>{unmappedCount}</strong><small>{unmappedCount ? "Needs mapping" : "No unmapped rates"}</small></div></div><div className="tax-summary-card"><div className="tax-summary-icon blue"><Icon type="cash-dollar" /></div><div className="tax-summary-copy"><span>Default Tax Account</span><strong>{taxName(defaultTax)}</strong><small>{defaultTax ? `(${taxId(defaultTax)})` : "Not configured"}</small></div></div></div>{data.taxSyncError && <div className="tax-info"><Icon type="info" />Zoho tax data could not be refreshed. Showing the last cached tax data, if available.</div>}<div className="tax-workspace"><div className="tax-tabs"><button className={`tax-tab ${tab === "mapping" ? "active" : ""}`} type="button" onClick={() => setTab("mapping")}>Tax Rate Mapping</button><button className={`tax-tab ${tab === "settings" ? "active" : ""}`} type="button" onClick={() => setTab("settings")}>Tax Settings</button></div>{tab === "mapping" ? <><div className="tax-toolbar"><button className="tax-add" type="button"><Icon type="plus" />Add Custom Mapping</button></div>{rows.length === 0 ? <div className="tax-empty">No Shopify tax rates detected.</div> : <div className="tax-table"><div className="tax-table-head"><span>Shopify Tax Rate</span><span>Shopify Tax Code</span><span>Zoho Books Tax Account</span><span>Tax Type</span><span>Status</span><span>Action</span></div>{rows.map((row) => { const selected = selectedTax(row); const selectedId = settings.rateMap?.[row.key] || ""; return <div className="tax-table-row" key={row.key}><strong>{row.label || row.key}</strong><span className="tax-code">{codeFor(row)}</span><select className="tax-select" name={`taxrate:${row.key}`} defaultValue={selectedId}><option value="">Not mapped</option>{taxes.map((tax) => <option key={taxId(tax)} value={taxId(tax)}>{taxName(tax)}{taxRate(tax) !== undefined && taxRate(tax) !== null ? ` (${taxRate(tax)}%)` : ""}</option>)}</select><span className="tax-type">{taxType(selected)}</span><span className={`tax-status ${selectedId ? "mapped" : "unmapped"}`}>{selectedId ? "Mapped" : "Not mapped"}</span><button className="tax-action" type="button" aria-label={`Actions for ${row.label || row.key}`}>⋮</button></div>; })}</div>}<div className="tax-bottom-note"><Icon type="info" />Tax mappings will be used for all future syncs. You can update mappings at any time.</div></> : <div className="tax-settings-panel"><div className="tax-setting-card"><h3>Default Tax Account</h3><p>Select the default Zoho Books tax account used when a Shopify tax rate has no specific mapping.</p><div className="tax-field"><label>Default Zoho Tax</label><select name="defaultTaxId" defaultValue={settings.defaultTaxId || ""}><option value="">Select tax account</option>{taxes.map((tax) => <option key={taxId(tax)} value={taxId(tax)}>{taxName(tax)}{taxRate(tax) !== undefined && taxRate(tax) !== null ? ` (${taxRate(tax)}%)` : ""}</option>)}</select></div></div><div className="tax-setting-card"><h3>Tax Calculation Preferences</h3><p>Choose how tax should be handled when syncing Shopify orders to Zoho Books.</p><label className="tax-checkbox"><input type="checkbox" name="pricesIncludeTax" value="true" defaultChecked={Boolean(settings.pricesIncludeTax)} />Prices include tax</label><label className="tax-checkbox"><input type="checkbox" name="discountBeforeTax" value="true" defaultChecked={Boolean(settings.discountBeforeTax)} />Apply discounts before tax</label></div></div>}<div className="tax-save"><button className="primary-btn" type="submit">Save Changes</button></div></div></Form></div>;
}

function WarehouseSection({ data }) {
  const locations = data.locations || []; const warehouses = data.warehouses || []; const mappings = data.warehouseMappings || {}; const mappedCount = locations.filter((location) => Boolean(mappings[location.id])).length;
