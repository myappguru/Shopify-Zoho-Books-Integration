...
import { getProductMappings, PRODUCTS_QUERY, normalizeProductNode } from "../models/productSync.server";
...
export const action = async ({ request }) => {
  ...
  await runInventoryPull({
    admin,
    shop,
    zohoAuth: { accessToken: token.accessToken, apiDomain: token.apiDomain, organizationId: connection.organization_id },
  });
  return null;
};
...
function getStatus(quantity) {
  ...
  if (quantity <= 10) return { label: "Low Stock", className: "low" };
...
export default function InventoryPage() {
  ...
  return (
    <s-page heading="Inventory" inlineSize="large">
      <style>{`
        .inventory-shell{width:100%;max-width:1260px;margin:0 auto;padding:0 0 24px;box-sizing:border-box;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif;color:#17233c}
      ...
      <div className="inventory-shell">
        ...
        <div className="inventory-table-card">
          <div className="inventory-toolbar">
            <label className="inventory-search"><Icon type="search" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search by product name, SKU or handle..." aria-label="Search inventory" /></label>
            <select className="inventory-select" value={locationFilter} onChange={(event) => setLocationFilter(event.target.value)} aria-label="Filter by location"><option value="all">All Locations</option>{data.locations.map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}</select>
            <select className="inventory-select" value={warehouseFilter} onChange={(event) => setWarehouseFilter(event.target.value)} aria-label="Filter by warehouse"><option value="all">All Warehouses</option>{warehouseOptions.map((warehouse) => <option key={warehouse} value={warehouse}>{warehouse}</option>)}</select>
            <button className={`inventory-filter-button ${statusFilter !== "all" ? "active" : ""}`} type="button" onClick={() => setStatusFilter(statusFilter === "all" ? "in" : "all")}><Icon type="filter" />Filters{statusFilter !== "all" ? " · 1" : ""}</button>
            <button className="inventory-export" type="button" onClick={() => window.print()}><Icon type="download" />Export</button>
          </div>

          <div className="inventory-tabs">
            <button className={`inventory-tab ${activeTab === "product" ? "active" : ""}`} type="button" onClick={() => setActiveTab("product")}>Inventory by Product</button>
            <button className={`inventory-tab ${activeTab === "location" ? "active" : ""}`} type="button" onClick={() => setActiveTab("location")}>Inventory by Location</button>
            <button className={`inventory-tab ${activeTab === "warehouse" ? "active" : ""}`} type="button" onClick={() => setActiveTab("warehouse")}>Inventory by Warehouse</button>
          </div>

          {activeTab === "product" ? <div className="inventory-table-wrap"><table className="inventory-table"><thead><tr><th>Product</th><th>SKU</th><th>Shopify Location</th><th>Zoho Warehouse</th><th>Available Qty</th><th>Incoming Qty</th><th>Committed Qty</th><th>Status</th><th>Last Updated</th><th></th></tr></thead><tbody>
            {filteredRows.map((row) => { const status = getStatus(row.available); return <tr key={row.id}>
              <td><div className="inventory-product">{row.imageUrl ? <img className="inventory-product-image" src={row.imageUrl} alt="" /> : <div className="inventory-product-image"></div>}<div style={{ minWidth: 0 }}><div className="inventory-product-name">{row.title}</div>{row.sku !== "—" && <div className="inventory-product-variant">{row.sku}</div>}</div></div></td>
              <td><span className="inventory-sku">{row.sku}</span></td><td>{row.locationName}</td><td>{row.warehouseName}</td><td><span className={`inventory-qty ${status.className}`}>{formatNumber(row.available)}</span></td><td>{formatNumber(row.incoming)}</td><td>{formatNumber(row.committed)}</td><td><span className={`inventory-status ${status.className}`}><span className="inventory-status-dot"></span>{status.label}</span></td><td><span className="inventory-date">{formatDate(row.updatedAt)}</span></td><td><button className="inventory-menu" type="button" aria-label={`Actions for ${row.title}`}>⋮</button></td>
            </tr>; })}
          </tbody></table>{filteredRows.length === 0 && <div className="inventory-empty">No inventory items match your current filters.</div>}</div> : <div className="inventory-empty">{activeTab === "location" ? "Location-level inventory view is ready for the next data layer." : "Warehouse-level inventory view is ready for the next data layer."}</div>}
