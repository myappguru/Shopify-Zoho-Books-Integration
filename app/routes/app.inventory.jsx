...
const PRODUCTS_BACKWARD_QUERY = `#graphql
  ...
  ${PRODUCT_FIELDS}
`;
const PRODUCTS_LAST_QUERY = `#graphql
  query InventoryProductsLast($first: Int!) {
    products(first: $first, reverse: true) { edges { node { ...InventoryProductFields } } pageInfo { hasNextPage hasPreviousPage startCursor endCursor } }
  }
  ${PRODUCT_FIELDS}
`;
...
export const loader = async ({ request }) => {
  ...
  if (lastPage) productsResponse = await admin.graphql(PRODUCTS_LAST_QUERY, { variables: { first: PAGE_SIZE } });
  else if (before) productsResponse = await admin.graphql(PRODUCTS_BACKWARD_QUERY, { variables: { last: PAGE_SIZE, before } });
  else productsResponse = await admin.graphql(PRODUCTS_FORWARD_QUERY, { variables: { first: PAGE_SIZE, after } });
  ...
  if (productsJson.errors?.length) {
    return { ...base, inventoryRows: [], stats: { totalInventory: 0, lowStock: 0, outOfStock: 0, inventoryValue: 0 }, pagination: { page, totalPages, hasNextPage: false, hasPreviousPage: false, startCursor: null, endCursor: null }, loadError: productsJson.errors.map((error) => error.message).join(" ") };
  }
  const productsConnection = productsJson.data?.products || {};
  const inventoryRows = [];
  let totalInventory = 0, lowStock = 0, outOfStock = 0, inventoryValue = 0;
  const productEdges = lastPage ? [...(productsConnection.edges || [])].reverse() : (productsConnection.edges || []);
...
export default function InventoryPage() {
  ...
  const { page, totalPages, hasNextPage, hasPreviousPage } = data.pagination;
  ...
  const previousHref = hasPreviousPage ? (page === 2 ? pageHref(1) : pageHref(page - 1, { before: data.pagination.startCursor })) : "";
  const nextHref = hasNextPage ? pageHref(page + 1, { after: data.pagination.endCursor }) : "";
      ...
      <div className="inventory">
        <div className="head">
          ...
          <div className="actions">
            <span className="connected"><span className="connected-dot" />{data.connected ? "Connected" : "Not connected"}</span>
            <Form method="get"><button className="refresh" type="submit" title="Refresh"><Icon type="refresh" /></button></Form>
            <div className="store-wrap">
              <button className="store" type="button" onClick={() => setStoreOpen((open) => !open)} aria-expanded={storeOpen}><span className="store-left"><Icon type="store" /><span>My Shopify Store</span></span><span className={`store-chevron ${storeOpen ? "open" : ""}`}><Icon type="chevron-down" /></span></button>
        ...
        <div className={`card ${loadingPage ? "page-loading" : ""}`}>
          <div className="toolbar">
            ...
            <div className="export-wrap">
              ...
              {exportOpen && <div className="dropdown" style={{ minWidth: 190 }}><button className="dropdown-item" type="button" onClick={() => { downloadCsv(rows); setExportOpen(false); }}><Icon type="document" />Export as CSV</button><button className="dropdown-item" type="button" onClick={() => { downloadCsv(rows); setExportOpen(false); }}><Icon type="file" />Export current page</button></div>}
            </div>
          </div>
          ...
          <div className="footer"><div>Showing {from} to {to} of {number(data.totalProducts)} products</div><div className="pagination"><PageLink href={previousHref} aria-label="Previous page">‹</PageLink>{page > 1 && <PageLink href={pageHref(1)}>1</PageLink>}{page > 3 && <span>…</span>}{page > 1 && <span className="page-btn active">{page}</span>}{hasNextPage && totalPages > page + 1 && <PageLink href={nextHref}>{page + 1}</PageLink>}{hasNextPage && totalPages > page + 2 && <span>…</span>}{totalPages > 1 && page !== totalPages && <PageLink href={lastHref}>{totalPages}</PageLink>}<PageLink href={nextHref} aria-label="Next page">›</PageLink></div></div>
        </div>
      </div>
    </s-page>
  );
