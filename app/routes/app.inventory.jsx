...
const INVENTORY_PRODUCTS_QUERY = `#graphql
  query InventoryProducts($first: Int, $after: String, $last: Int, $before: String) {
    products(first: $first, after: $after, last: $last, before: $before) {
      edges { node { id title handle featuredMedia { preview { image { url altText } } } variants(first: 50) { edges { node { id title sku price inventoryQuantity inventoryItem { id inventoryLevels(first: 50) { edges { node { location { id name } quantities(names: ["available", "incoming", "committed"]) { name quantity } } } } } } } } } }
...
export const loader = async ({ request }) => {
  ...
  const variables = lastPage || before ? { last: PAGE_SIZE, before: before || null } : { first: PAGE_SIZE, after };
  const [productsResponse, locationsResponse, countResponse] = await Promise.all([
    admin.graphql(INVENTORY_PRODUCTS_QUERY, { variables }), admin.graphql(LOCATIONS_QUERY), admin.graphql(PRODUCT_COUNT_QUERY),
  ]);
  const [productsJson, locationsJson, countJson] = await Promise.all([productsResponse.json(), locationsResponse.json(), countResponse.json()]);
  ...
  for (const product of products) for (const variant of product.variants || []) {
    ...
    const incoming = Number(quantities.incoming || 0), committed = Number(quantities.committed || 0);
    const locationId = level?.location?.id || locations[0]?.id || null;
    const warehouseId = locationId ? warehouseMappings[locationId] : null;
    totalInventory += available; inventoryValue += available * Number(variant.price || 0);
...
function relative(value) { if (!value) return "Never"; const m = Math.floor(Math.max(0, Date.now() - new Date(value).getTime()) / 60000); return m < 1 ? "Just now" : m < 60 ? `${m} min ago` : m < 1440 ? `${Math.floor(m / 60)} hr ago` : `${Math.floor(m / 1440)} days ago`; }
...
function href(page, options = {}) { const p = new URLSearchParams(); if (page > 1) p.set("page", String(page)); if (options.after) p.set("after", options.after); if (options.before) p.set("before", options.before); if (options.last) p.set("last", "1"); return p.toString() ? `?${p}` : ""; }
...
export default function InventoryPage() {
  ...
  const [search, setSearch] = useState(""); const [location, setLocation] = useState("all"); const [warehouse, setWarehouse] = useState("all"); const [statusFilter, setStatusFilter] = useState("all"); const [tab, setTab] = useState("product");
  const showLog = useAutoDismiss(data.latestPullLog?.id); const isPulling = navigation.state === "submitting" && navigation.formData?.get("intent") === "pull-from-zoho"; const isLive = data.connected && data.inventoryAccountId && data.warehouseMappingCount > 0;
  const { page, totalPages, hasNextPage, hasPreviousPage, startCursor, endCursor } = data.pagination;
  const rows = useMemo(() => data.inventoryRows.filter((row) => { const q = search.trim().toLowerCase(); const s = status(row.available).key; return (!q || `${row.title} ${row.sku} ${row.locationName} ${row.warehouseName}`.toLowerCase().includes(q)) && (location === "all" || row.locationId === location) && (warehouse === "all" || row.warehouseName === warehouse) && (statusFilter === "all" || s === statusFilter); }), [data.inventoryRows, search, location, warehouse, statusFilter]);
