...
export const loader = async ({ request }) => {
  ...
  if (lastPage) {
    productsResponse = await admin.graphql(PRODUCTS_LAST_QUERY, { variables: { last: PAGE_SIZE } });
  } else if (before) {
  ...
  if (productsJson.errors?.length) {
    ...
    return {
      ...
      latestPullLog,
      inventoryRows: [], locations, totalProducts,
      stats: { totalInventory: 0, lowStock: 0, outOfStock: 0, inventoryValue: 0 },
      pagination: { page, totalPages: Math.max(1, Math.ceil(totalProducts / PAGE_SIZE)), hasNextPage: false, hasPreviousPage: false, startCursor: null, endCursor: null },
      loadError: message,
    };
  }
...
export default function InventoryPage() {
  ...
  const PageLink = ({ to: href, children, className = "page-btn", ...props }) => href ? <Link to={href} className={className} {...props}>{children}</Link> : <span className={`${className} disabled`} {...props}>{children}</span>;
  ...
  <div className="inventory">
    ...
    <div className={`card ${loadingPage ? "page-loading" : ""}`}>
      ...
      <div className="footer"><div>Showing {from} to {to} of {number(data.totalProducts)} products</div><div className="pagination"><PageLink href={previousHref} aria-label="Previous page">‹</PageLink>{page > 2 && <PageLink href={pageHref(1)}>1</PageLink>}{page > 3 && <span>…</span>}{page > 1 && <span className="page-btn active">{page}</span>}{hasNextPage && <PageLink href={nextHref}>{page + 1}</PageLink>}{hasNextPage && totalPages > page + 1 && <span>…</span>}{totalPages > 1 && page !== totalPages && <PageLink href={lastHref}>{totalPages}</PageLink>}<PageLink href={nextHref} aria-label="Next page">›</PageLink></div></div>
    </div>
  </div></s-page>;
}
