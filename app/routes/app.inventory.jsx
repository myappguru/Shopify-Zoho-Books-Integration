...
function getPaginationVariables({ after, before, lastPage }) {
  if (lastPage) return { last: PAGE_SIZE };
  if (before) return { last: PAGE_SIZE, before };
  return { first: PAGE_SIZE, after: after || null };
}
...
export const loader = async ({ request }) => {
  ...
  const variables = getPaginationVariables({ after, before, lastPage });
  ...
  if (productsJson.errors?.length) {
    console.error("Inventory GraphQL error:", productsJson.errors);
    return {
      ...
      latestPullLog,
      inventoryRows: [],
      locations: locationsJson.data?.locations?.nodes || [],
      totalProducts: Number(countJson.data?.productsCount?.count || 0),
      stats: { totalInventory: 0, lowStock: 0, outOfStock: 0, inventoryValue: 0 },
      pagination: { page, totalPages: 1, hasNextPage: false, hasPreviousPage: false, startCursor: null, endCursor: null },
      loadError: productsJson.errors.map((error) => error.message).join(" "),
    };
  }

  const productsConnection = productsJson.data?.products || {};
  const productNodes = (productsConnection.edges || []).map((edge) => edge.node).filter(Boolean);
  ...
  for (const product of productNodes) {
    ...
    for (const variant of product.variants?.edges?.map((edge) => edge.node).filter(Boolean) || []) {
      const levels = variant.inventoryItem?.inventoryLevels?.edges?.map((edge) => edge.node).filter(Boolean) || [];
  ...
  return {
    ...
    pagination: {
      ...
      hasPreviousPage: Boolean(productsConnection.pageInfo?.hasPreviousPage),
      startCursor: productsConnection.pageInfo?.startCursor || null,
...
function downloadCsv(rows) {
  ...
  anchor.href = url;
  anchor.download = `inventory-page-${Date.now()}.csv`;
  anchor.click();
...
export default function InventoryPage() {
  ...
  return <s-page heading="Inventory" inlineSize="large">
    ...
    <div className="inventory">
      ...
      <div className="stats">
        <Stat label="Total Products" value={number(data.totalProducts)} caption="All products" icon="product" tone="blue" />
        <Stat label="Total Inventory" value={number(data.stats.totalInventory)} caption="Across loaded locations" icon="inventory" tone="green" />
        <Stat label="Low Stock" value={number(data.stats.lowStock)} caption="Below threshold" icon="alert-triangle" tone="orange" />
        <Stat label="Out of Stock" value={number(data.stats.outOfStock)} caption="Out of stock" icon="x-circle" tone="red" />
        <Stat label="Inventory Value" value={currency(data.stats.inventoryValue)} caption="Estimated value" icon="money" tone="purple" />
      </div>
      ...
      <div className={`card ${navigation.state === "loading" ? "page-loading" : ""}`}>
        <div className="toolbar">
          <label className="search"><Icon type="search" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search by product name, SKU or handle..." /></label>
          <select className="select" value={location} onChange={(event) => setLocation(event.target.value)}><option value="all">All Locations</option>{data.locations.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>
          <select className="select" value={warehouse} onChange={(event) => setWarehouse(event.target.value)}><option value="all">All Warehouses</option>{warehouses.map((item) => <option key={item}>{item}</option>)}</select>
          <button className="filter" type="button" onClick={() => setStatusFilter(statusFilter === "all" ? "in" : "all")}><Icon type="filter" />Filters{statusFilter !== "all" ? " · 1" : ""}</button>
          <div className="export-wrap">
            <button className="export export-button" type="button" onClick={() => setExportOpen((open) => !open)} aria-expanded={exportOpen}><Icon type="download" />Export<span className="export-chevron"><Icon type="chevron-down" /></span></button>
            {exportOpen && <div className="dropdown export-dropdown"><button className="dropdown-item" type="button" onClick={() => { downloadCsv(rows); setExportOpen(false); }}><Icon type="document" />Export as CSV</button><button className="dropdown-item" type="button" onClick={() => { downloadCsv(rows); setExportOpen(false); }}><Icon type="file" />Export current page</button></div>}
          </div>
        </div>

        <div className="tabs">
          <button className={`tab ${tab === "product" ? "active" : ""}`} type="button" onClick={() => setTab("product")}>Inventory by Product</button>
          <button className={`tab ${tab === "location" ? "active" : ""}`} type="button" onClick={() => setTab("location")}>Inventory by Location</button>
          <button className={`tab ${tab === "warehouse" ? "active" : ""}`} type="button" onClick={() => setTab("warehouse")}>Inventory by Warehouse</button>
        </div>

        {tab === "product" ? <div className="table-wrap"><table className="table"><thead><tr><th>Product</th><th>SKU</th><th>Shopify Location</th><th>Zoho Warehouse</th><th>Available Qty</th><th>Incoming Qty</th><th>Committed Qty</th><th>Status</th><th>Last Updated</th><th></th></tr></thead><tbody>
          {rows.map((row) => { const currentStatus = status(row.available); return <tr key={row.id}><td><div className="product">{row.imageUrl ? <img className="image" src={row.imageUrl} alt="" /> : <div className="image" />}<strong>{row.title}</strong></div></td><td>{row.sku}</td><td>{row.locationName}</td><td>{row.warehouseName}</td><td><span className={`qty ${currentStatus.key}`}>{number(row.available)}</span></td><td>{number(row.incoming)}</td><td>{number(row.committed)}</td><td><span className={`badge ${currentStatus.key}`}>{currentStatus.label}</span></td><td>{date(row.updatedAt)}</td><td><button className="row-menu" type="button" aria-label={`Actions for ${row.title}`}>⋮</button></td></tr>; })}
        </tbody></table>{rows.length === 0 && <div className="empty">No inventory items match your current filters.</div>}</div> : <div className="empty">{tab === "location" ? "Location-level inventory view is ready for the next data layer." : "Warehouse-level inventory view is ready for the next data layer."}</div>}

        <div className="footer">
          <div>Showing {from} to {to} of {number(data.totalProducts)} products</div>
          <div className="pagination">
            <a className={`page-btn ${!hasPreviousPage ? "disabled" : ""}`} href={previousHref} aria-label="Previous page">‹</a>
            {page > 2 && <a className="page-btn" href={pageHref(1)}>1</a>}
            {page > 3 && <span>…</span>}
            {page > 1 && <span className="page-btn active">{page}</span>}
            {hasNextPage && <a className="page-btn" href={nextHref}>{page + 1}</a>}
            {hasNextPage && totalPages > page + 1 && <span>…</span>}
            {totalPages > 1 && page !== totalPages && <a className="page-btn" href={lastHref}>{totalPages}</a>}
            <a className={`page-btn ${!hasNextPage ? "disabled" : ""}`} href={nextHref} aria-label="Next page">›</a>
          </div>
        </div>
      </div>
    </div>
